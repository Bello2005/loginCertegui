...
const MaterialesModal = ({ isOpen, onClose, material = null, onSave }) => {
  const [formData, setFormData] = useState({ nombre: "", descripcion: "", unidad: "", precio_unitario: "" });
  ...
  useEffect(() => {
    if (material) {
      ...
      setFormData({ nombre: "", descripcion: "", unidad: "", precio_unitario: "" });
    }
    setErrors({});
  }, [material, isOpen]);
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-white">Unidad</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-white">Precio Unitario</label>
                <input
                  type="number"
                  step="0.01"
...
const MaterialesDashboard = () => {
  ...
  const fetchMateriales = (page = 1) => {
    setLoading(true);
    api.get(`api/materiales?page=${page}&limit=10`)
      .then(res => {
  ...
  const handleSaveMaterial = async (formData) => {
    if (editingMaterial) {
      await api.put(`api/materiales/${editingMaterial.id}`, formData);
    } else {
      await api.post('api/materiales', formData);
    }
    fetchMateriales(pagination.page);
  };

  const eliminarMaterial = (id) => {
    if (window.confirm("¿Eliminar este material?")) {
      api.delete(`api/materiales/${id}`)
        .then(() => {
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <main className="flex-1 flex flex-col gap-8 px-8 overflow-y-auto overflow-x-hidden min-w-0">
          <div className="p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-md">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Materiales</h2>
              <button 
              ...
              >
                <UserPlus className="w-5 h-5" /> Nuevo Material
              </button>
            </div>
                  ...
                  <table className="min-w-full text-sm text-left">
                    <thead className="bg-gray-100 dark:bg-gray-800">
                      <tr>
                        ...
                        <th className="px-6 py-3 font-semibold text-gray-900 dark:text-white">Unidad</th>
                        <th className="px-6 py-3 font-semibold text-gray-900 dark:text-white">Precio Unitario</th>
                        <th className="px-6 py-3 text-right font-semibold text-gray-900 dark:text-white">Acciones</th>
                      </tr>
                    </thead>
                    <tbody>
                      {materiales.map(m => (
                        <tr key={m.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{m.nombre}</td>
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{m.unidad || "-"}</td>
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{m.precio_unitario ? `$${parseFloat(m.precio_unitario).toFixed(2)}` : "-"}</td>
                          <td className="px-6 py-4 text-right flex justify-end gap-3">
                            <button onClick={() => handleOpenModal(m)} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors">
                              <Edit3 className="w-5 h-5" />
                            </button>
                            <button onClick={() => eliminarMaterial(m.id)} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 transition-colors">
                ...
                {pagination.totalPages > 1 && (
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => fetchMateriales(pagination.page - 1)}
...
export default MaterialesDashboard;
