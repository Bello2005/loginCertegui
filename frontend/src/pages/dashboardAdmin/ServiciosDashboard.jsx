...
const ServiciosModal = ({ isOpen, onClose, servicio = null, onSave }) => {
  const [formData, setFormData] = useState({ nombre: "", descripcion: "", precio: "", categoria: "", activo: 1 });
  ...
  useEffect(() => {
    if (servicio) {
      ...
      setFormData({ nombre: "", descripcion: "", precio: "", categoria: "", activo: 1 });
    }
    setErrors({});
  }, [servicio, isOpen]);
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-white">Precio *</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="block text-sm font-semibold mb-2 text-gray-700 dark:text-white">Categoría</label>
                <input
                  type="text"
...
const ServiciosDashboard = () => {
  ...
  const handleSaveServicio = async (formData) => {
    ...
    fetchServicios(pagination.page);
  };
        ...
        <main className="flex-1 flex flex-col gap-8 px-8 overflow-y-auto overflow-x-hidden min-w-0">
          <div className="p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-md">
            <div className="flex items-center justify-between mb-6">
              ...
              >
                <UserPlus className="w-5 h-5" /> Nuevo Servicio
              </button>
            </div>
              ...
              <>
                <div className="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-xl">
                  <table className="min-w-full text-sm text-left">
                    ...
                    <tbody>
                      {servicios.map(s => (
                        <tr key={s.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          ...
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">${parseFloat(s.precio).toFixed(2)}</td>
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{s.activo ? "Activo" : "Inactivo"}</td>
                          <td className="px-6 py-4 text-right flex justify-end gap-3">
                            <button onClick={() => handleOpenModal(s)} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors">
                              <Edit3 className="w-5 h-5" />
                            </button>
                            <button onClick={() => eliminarServicio(s.id)} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 transition-colors">
