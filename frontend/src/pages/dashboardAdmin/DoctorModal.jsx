...
import { X, UserPlus, Stethoscope, Mail, Lock, Edit } from "lucide-react";
...
const DoctorModal = ({ isOpen, onClose, doctor = null, onSave }) => {
  ...
  useEffect(() => {
    if (doctor) {
      // Si es edición, separar nombre completo
      const nombreParts = doctor.nombre.split(" ");
      setFormData({
      ...
      // Resetear formulario para nuevo doctor
      setFormData({
        nombre: "",
        apellido: "",
        correo: "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Error al guardar el doctor");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white">
                {doctor ? <Edit className="w-5 h-5" /> : <UserPlus className="w-5 h-5" />}
              </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-indigo-500/20 transition-all bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white"
