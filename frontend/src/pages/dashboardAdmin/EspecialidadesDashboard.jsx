...
const EspecialidadesModal = ({ isOpen, onClose, especialidad = null, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(especialidad ? "Especialidad actualizada" : "Especialidad creada");
      onClose();
    } catch (error) {
...
const EspecialidadesDashboard = () => {
  ...
  const handleSaveEspecialidad = async (formData) => {
    if (editingEspecialidad) {
      await api.put(`api/especialidades/${editingEspecialidad.id}`, formData);
    } else {
      await api.post('api/especialidades', formData);
    }
    fetchEspecialidades(pagination.page);
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <main className="flex-1 flex flex-col gap-8 px-8 overflow-y-auto overflow-x-hidden min-w-0">
          <div className="p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-md">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Especialidades</h2>
              <button 
              ...
              >
                <UserPlus className="w-5 h-5" /> Nueva Especialidad
              </button>
            </div>
                    ...
                    <tbody>
                      {especialidades.map(e => (
                        <tr key={e.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          ...
                          <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{e.descripcion || "-"}</td>
                          <td className="px-6 py-4 text-right flex justify-end gap-3">
                            ...
                            <button onClick={() => eliminarEspecialidad(e.id)} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 transition-colors">
